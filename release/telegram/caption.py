...
def build_blocks(env):
    ...
    return block_aetherium, block_root, block_addons, footer
...
def build_channel_caption(env, variant_links, variant_versions=None):
    ...
    sections = [
        f"*Aetherium \\| Build \\| {mdv2_escape(linux_ver)}*\n"
    ...
    group_url = mdv2_escape_url("https://t.me/AetheriumKernel".format(env.get("TELEGRAM_GROUP", "")))
    sections.append(
        f"Found a bug? Let's discuss it in [Aetherium CHAT]({group_url})"
    )
...
#    sections.append(f"{date_str} \u00b7 [Support]({donate_url})")
    ...
    sections.append("\\#GKI \\#AetheriumKernel")
...
def main():
    ...
    block_aetherium, block_root, block_addons, footer = build_blocks(env)

    caption_group = "\n".join([block_aetherium, block_root, block_addons, footer])
