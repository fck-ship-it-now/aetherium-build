...
ZIP_NAME="Aetherium-${KERNEL_VERSION}.${SUBLEVEL}-${ZIP_VARIANT_TAG}-R${GITHUB_RUN_NUMBER:-0}.zip"
export ZIP_NAME
...
if [ "${USE_AK3_CACHE}" = "true" ] && [ -d "${HOME}/ak3-cache" ]; then
    ...
    retry 3 run_quiet git clone -q --depth=1 -b master \
        https://github.com/kaminarich/AnyKernel3.git "$TOOL_AK3_DIR" \
